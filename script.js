/*      GODGAHNUM.JS
gnum format: {sign : -1/0/1 , array : [any]}
*/

/*-----------------------Ultilizer------------------------*/
function getMaxCallStackSize(){let i = 0;function recurse(){i++;recurse();}try{recurse();}catch(e){return i;}}


/*-----------------------EDITABLE CONSTRAINT-------------------------------*/
var max_iterations = getMaxCallStackSize() //maximum iterations for lowerentry function when reducing a gnum
var detail_level = 2
/*
0 : show nothing
1 : show unhandlable errors
2 : show everything
*/


/*----------------------UNEDITABLE CONSTRAINT----------------------------*/

const gnum_maxiterations = { sign: 1, array: [max_iterations] }
const inf = { sign: 1, array: [10 ** 308.254] }
const one = { sign: 1, array: [1] }
const zero = { sign: 0, array: [0] }
const two = { sign: 1, array: [2] }
...
/*
dependencies: none
use to flat 2 entry array
*/
function flatenarr(arr) {
    if (!arr || arr.length > 2 || arr.length == 0) return undefined
    let flatarr = []
    for (let i = 0; i < arr.length; i++)
        flatarr = typeof arr[i] === "number" ? flatarr.concat(arr[i]) : flatarr.concat(flatenarr(arr[i]))
    return flatarr
}

/*
dependencies: [flatenarr, break_eternitynum.js]
use to caculate 2 entry arr in break_eternity.js
extended for 1 entry too
*/
function arr2val(arr) {
    if (!arr || arr.length > 2 || arr.length == 0) return undefined
    if (arr.length == 1) return Decimal.fromNumber(arr[0])
    const processed = flatenarr(arr)
    let val = processed[processed.length - 1]
    for (let i = processed.length - 2; i >= 0; i--) val = Decimal.pow(processed[i], val)
    return val
}

/*
dependencies: [arr2val, break_eternitynum.js]
use to convert 2 entry to 1e1e1e1e...ex format (x > 1)
extended for 1 entry too
*/
function to10ex(gnum) {
    if (!gnum || gnum.array.length > 2 || Math.abs(gnum.sign) > 1 || Math.ceil(gnum.sign) != gnum.sign || gnum.array.length == 0) return undefined
    return (gnum.sign > -1 ? "" : "-") + "1e".repeat(arr2val(gnum.array).layer) + arr2val(gnum.array).mag
}


/*
dependencies: [to10ex, break_eternitynum.js]
use to compare 1 entry with all ban
*/
function cmp1entry(a, b) {
    if (!a || !b || a.array.length > 1 || (Math.abs(a.sign) > 1) || (Math.abs(b.sign) > 1) || (Math.ceil(a.sign) != a.sign) || (Math.ceil(b.sign) != b.sign) || !a.array.length || !b.array.length) return undefined
    if (a.sign > b.sign) return 1
    if (a.sign < b.sign) return -1
    if (a.sign == 0 && b.sign == 0) return 0
    if (b.array.length == 1) return a.array[0] > b.array[0] ? 1 * a.sign : (a.array[0] < b.array[0] ? -1 * a.sign : 0)
    if (b.array.length == 2) {
        if (Decimal.lt(a.array[0], to10ex(b))) return -1 * a.sign
        if (Decimal.gt(a.array[0], to10ex(b))) return 1 * a.sign
        return 0
    }
    return -1 * a.sign
}
...
/*
dependencies: [to10ex, break_eternitynum.js]
forced compare 2 entry, both input must be 2 entry to make sure they are work
*/
function fcmp2entry(a, b) {
    if (!a || !b || a.array.length != 2 || b.array.length != 2 || Math.abs(a.sign) > 1 || Math.abs(b.sign) > 1 || Math.ceil(a.sign) != a.sign || Math.ceil(b.sign) != b.sign || !a.array.length || !b.array.length) return undefined
    if (a.sign > b.sign) return 1
    if (a.sign < b.sign) return -1
    if (a.sign == 0 && b.sign == 0) return 0
    if (Decimal.lt(to10ex(a), to10ex(b))) return -1 * a.sign
    if (Decimal.gt(to10ex(a), to10ex(b))) return 1 * a.sign
    return 0
}
...
/*
dependencies: none
used to nesting arrays
written by chatgpt (verified)
*/
function nesting(arr, index, iterate) {
    let nested = arr
    while (iterate-- > 0) {
        let temp = [...arr]
        temp[index] = nested
...
/*
dependencies: none
check if it a flat array
*/
function isflat(arr) {
    for (let i = 0; i < arr.length; i++) if (Array.isArray(arr[i])) return false
    return true
}
...
compare 2 flatarr
*/
function cmp2flatarr(a, b) {
    if (!a || !b || !isflat(a) || !isflat(b) || Math.abs(a.sign) > 1 || Math.abs(b.sign) > 1 || Math.ceil(a.sign) != a.sign || Math.ceil(b.sign) != b.sign || !a.array.length || !b.array.length) return undefined
    if (a.sign > b.sign) return 1
    if (a.sign < b.sign) return -1
    if (a.sign == 0 && b.sign == 0) return 0
    if (a.array.length > b.array.length) return 1 * a.sign
    if (a.array.length < b.array.length) return -1 * a.sign
    for (let i = a.array.length - 1; i >= 0; i--) {
        if (a.array[i] > b.array[i]) return 1 * a.sign
        if (a.array[i] < b.array[i]) return -1 * a.sign
    }
    return 0
}
...
/*
dependencies: [unwarplen1arr]
sometimes, there would be a single element array inside,we should fix that
written by chatgpt (verified)
*/
function unwarplen1arr(arr) {
    while (Array.isArray(arr) && arr.length === 1 && Array.isArray(arr[0])) arr = arr[0]
    return arr.map(el => {
        if (Array.isArray(el)) {
            el = unwarplen1arr(el)
            while (Array.isArray(el) && el.length === 1 && Array.isArray(el[0])) el = el[0]
        }
        return el
    })
}
...
/*
dependencies: [cmp1entry, arr2val, break_eternitynum.js]
convert gnum to number (if possible)
*/
function tonumber(gnum) {
    if (!gnum.array || gnum.array.length > 2 || Math.abs(gnum.sign) > 1 || Math.ceil(gnum.sign) != gnum.sign || !gnum.array.length || cmp1entry(inf, { sign: 1, array: gnum.array }) == -1) return undefined
    if (gnum.array.length == 1) return gnum.sign * gnum.array[0]
    if (gnum.array.length == 2) return Number(Decimal.mul(gnum.sign, arr2val(gnum.array)))
}

/*
dependencies: [arrrule2]
rule 2 in Bird array notaion (remove trailling 1)
*/
function arrrule2(arr) {
    const trimmed = arr.map(item => Array.isArray(item) ? arrrule2(item) : item)
    let i = trimmed.length
    while (i > 0 && trimmed[i - 1] === 1) i--
    return trimmed.slice(0, i)
}

/*
dependencies: [arrrule3]
rule 3 in Bird array notaion
*/
function arrrule3(arr) {
    if (!Array.isArray(arr)) return arr
    let cleaned = arr.map(arrrule3)
    return cleaned[1] === 1 ? [cleaned[0]] : cleaned
}

/*
dependencies: [unwarplen1arr, arrrule2]
rule 2 but built for gnum
*/
function rule2(gnum) {
    if (Math.abs(gnum.sign) > 1 || Math.ceil(gnum.sign) != gnum.sign || !gnum.array || !gnum.array.length) return undefined
    return { sign: gnum.sign, array: unwarplen1arr(arrrule2(gnum.array)) }
}

/*
dependencies: [unwarplen1arr, arrrule3]
rule 3 but built for gnum
*/
function rule3(gnum) {
    if (Math.abs(gnum.sign) > 1 || Math.ceil(gnum.sign) != gnum.sign || !gnum.array || !gnum.array.length) return undefined
    return { sign: gnum.sign, array: unwarplen1arr(arrrule3(gnum.array)) }
}

/*
dependencies: [cmp1entry, tonumber, rule2, rule3, nesting, unwarplen1arr]
bottom case of lowerentry
*/
function lowerentrybtm(gnum, index) {
    if (Math.abs(gnum.sign) > 1 || Math.ceil(gnum.sign) != gnum.sign || !gnum.array || !gnum.array.length || index >= gnum.array.length || index < 0) return undefined
    if (cmp1entry(one, { sign: 1, array: unwarplen1arr([gnum.array[index]]) }) == 0) return undefined
    if (gnum.array.length < 3) return undefined
    let arr = gnum.array
    let start = Array(index).fill(arr[0])
    let end = arr.slice(index + 1)
    let reduced = tonumber({ sign: 1, array: unwarplen1arr([arr[index]]) }) - 1
    let iteration = tonumber({ sign: 1, array: unwarplen1arr([arr[1]]) }) - 1
    if (!iteration || !reduced) return undefined
    if (cmp1entry(gnum_maxiterations, { sign: 1, array: [iteration] }) == -1) return undefined
    let bottom = [...start, reduced, ...end]
    return { sign: gnum.sign, array: unwarplen1arr(rule3(rule2({ sign: gnum.sign, array: nesting(bottom, index - 1, iteration) })).array) }
}

/*
dependencies: [cmp1entry, lowerentrybtm]
use to lower entries
*/
function lowerentry(gnum, index) {
    if (Math.abs(gnum.sign) > 1 || Math.ceil(gnum.sign) != gnum.sign || !gnum.array || !gnum.array.length || index >= gnum.array.length || index < 0) return undefined
    if (cmp1entry(one, { sign: 1, array: unwarplen1arr([gnum.array[index]]) }) == 0) return undefined
    if (gnum.array.length < 3) return undefined
    let i = index - 1, current = gnum
    while (i > 1) {
        if (current.array[i] != 1) {
            if (current !== undefined) {
                while (current.array?.[i] !== 1) {
                    current = lowerentrybtm(current, i)
                    if (!current) break
                }
            }
    ...
    return lowerentrybtm(current, index)
}
/*
dependencies: none
use to convert power pairs to number (if they are not overflowing infinity)
*/
function convertPowerPairs(arr) {
    if (!Array.isArray(arr)) return arr
    if (arr.length === 2 && typeof arr[0] === 'number' && typeof arr[1] === 'number') {
        const val = Math.pow(arr[0], arr[1])
        return (val !== Infinity && !Number.isNaN(val)) ? val : arr
    }
    return arr.map(element => convertPowerPairs(element))
}

/*
dependencies: [convertPowerPairs, unwarplen1arr, isflat, cmp2flatarr, cmp1entry, fcmp2entry, to10ex, lowerentry, break_eternitynum.js]
use to compare 2 gnums.Though standing alone have some limitations for 3+ entries like
- cant consistently compare with base =! 10
- cant consistently compare with recursive array have higher grow rates
- cant consistently compare with non - natural number element
*/
function cmp(a, b) {
    // Catching unsupported cases
    if (!a || !b || Math.abs(a.sign) > 1 || Math.abs(b.sign) > 1 ||
        Math.ceil(a.sign) != a.sign || Math.ceil(b.sign) != b.sign ||
        !a.array.length || !b.array.length) return undefined

    //fix both side array
    let convertedA = convertPowerPairs(unwarplen1arr(a.array))
    let convertedB = convertPowerPairs(unwarplen1arr(b.array))
    a = { sign: a.sign, array: (Array.isArray(convertedA) ? convertedA : [convertedA]) }
    b = { sign: b.sign, array: (Array.isArray(convertedB) ? convertedB : [convertedB]) }
    ...
    // Case 2: Both are flat
    if (isflat(a.array) && isflat(b.array)) return cmp2flatarr(a, b)

    // Case 3: Entry length special handling
    // Case 3.1: a has only 1 entry
    if (a.array.length == 1) return cmp1entry(a, b)

    // Case 3.2: b has only 1 entry (symmetric)
    if (b.array.length == 1) return cmp1entry(b, a) * -1

    // Case 3.3: a , b has exactly 2 entries
    if ((a.array.length == 2) && (b.array.length == 2)) return fcmp2entry(a, b)

    // Case 4: Length difference
    // Case 4.1: len(a) - len(b) > 1
    /*
    explain a bit here,you need papers too
    try to lower {10,2,1,1,2} to {10,10,{10,10,{10,10,{10,10,...}}}}
    even you have graham's number,it still can't do it
    */
    if (a.array.length - b.array.length > 1) return 1 * a.sign
    if (b.array.length - a.array.length > 1) return -1 * a.sign

    // Case 4.2: len(a) - len(b) == 1
    /*
    explain a bit here too
    if the format is {10,2,1,1,1,1,...,2}, it is possible to lower it to {10,10,10,...,{10,10,10,...}}
    if the format is {10,2,2,1,1,...,1}, it overtakes Graham’s number to lower it
    */

    // Case 4.2.1: a is longer
    if (a.array.length - b.array.length == 1) {
        if (cmp1entry(two, { sign: 1, array: unwarplen1arr([a.array[a.array.length - 1]]) }) == -1) return 1 * a.sign

        for (let i = a.array.length - 2; i >= 2; i--) { if (a.array[i] != 1) return 1 * a.sign }
        let aNew = lowerentry(a, a.array.length - 1)
        if (aNew === undefined) return 1 * a.sign
        return cmp(aNew, b)

    }

    // Case 4.2.2: b is longer (symmetric)
    if (b.array.length - a.array.length == 1) {
        if (cmp1entry(two, { sign: 1, array: unwarplen1arr([b.array[b.array.length - 1]]) }) == -1) return -1 * a.sign
        for (let i = b.array.length - 2; i >= 2; i--) { if (b.array[i] != 1) return -1 * a.sign }

        let bNew = lowerentry(b, b.array.length - 1)
        if (bNew === undefined) return -1 * a.sign
        return cmp(a, bNew)
    }

    // Case 5: Equal length arrays
    // Iterate from right to left
    for (let i = a.array.length - 1; i > 0; i--) {
        if (a.array[i] != b.array[i]) {
            // Case 5.1: Both are numbers
            if ((typeof a.array[i] === "number") && (typeof b.array[i] === "number")) {
                //case 5.1.1: special case : i = 1
                if (i == 1) return (a.array[i] > b.array[i] ? 1 : -1) * a.sign

                // Case 5.2.1: Difference > 1
                if (Math.abs(a.array[i] - b.array[i]) > 1)
                    return (a.array[i] > b.array[i] ? 1 : -1) * a.sign

                // Case 5.2.2: Difference == 1
                if (a.array[i] > b.array[i]) {
                    let Anew = lowerentry(a, i)
                    if (Anew === undefined) return 1 * a.sign
                    a = Anew
                    continue
                }
            ...
            // Case 5.2: a[i] is 1-entry, b[i] is 2-entry
            if (a.array[i].length == 1 && b.array[i].length == 2) {
                let Adecimal = to10ex({ sign: 1, array: [a.array[i]] })
                let Bdecimal = to10ex({ sign: 1, array: [...b.array[i]] })
                if (Decimal.sub(Adecimal, Bdecimal).abs().gt(1)) return (Decimal.gt(Adecimal, Bdecimal) ? 1 : -1) * a.sign

                //special case : i = 1
                if (i == 1) {
                    if (Decimal.gt(Adecimal, Bdecimal)) return 1 * a.sign
                    if (Decimal.lt(Adecimal, Bdecimal)) return -1 * a.sign
                    return 0 //no less no more just equal
                }

                if (Decimal.gt(Adecimal, Bdecimal)) {
                    let Anew = lowerentry(a, i)
                    if (Anew === undefined) return 1 * a.sign
                    a = Anew
                    continue
                ...
                if (Decimal.lt(Adecimal, Bdecimal)) {
                    let Bnew = lowerentry(b, i)
                    if (Bnew === undefined) return -1 * a.sign
                    b = Bnew
                    continue
            ...
            // Case 5.3: a[i] is 2-entry, b[i] is 1-entry (symmetric)
            if (a.array[i].length == 2 && b.array[i].length == 1) {
                let Adecimal = to10ex({ sign: 1, array: [...a.array[i]] })
                let Bdecimal = to10ex({ sign: 1, array: [b.array[i]] })
                if (Decimal.sub(Adecimal, Bdecimal).abs().gt(1)) return (Decimal.gt(Adecimal, Bdecimal) ? 1 : -1) * a.sign
            ...
            // Case 5.4: both are 2-entry
            if (a.array[i].length == 2 && b.array[i].length == 2) {
                let Adecimal = to10ex({ sign: 1, array: [...a.array[i]] })
                let Bdecimal = to10ex({ sign: 1, array: [...b.array[i]] })
                if (Decimal.sub(Adecimal, Bdecimal).abs().gt(1)) return (Decimal.gt(Adecimal, Bdecimal) ? 1 : -1) * a.sign
            ...
            // Case 5.5: a[i] is 1-entry, b[i] is 3+ entries
            if (a.array[i].length == 1 && b.array[i].length > 2) {
                /*
                {10,2,2} is the smallest possible 2-entry value after rules
                and it's > any double-precision value
                */
                return -1 * a.sign
            }

            // Case 5.6: a[i] is 3+ entries, b[i] is 1-entry (symmetric)
            if (a.array[i].length > 2 && b.array[i].length == 1) return 1 * a.sign

            // Case 5.7: Both are 3+ entries, recurse
            // NOTE: recursive compare of sub-gnums — this is where deep structural recursion happens.
            let cmpVal = cmp(
                { sign: a.sign, array: Array.isArray(a.array[i]) ? [...a.array[i]] : [a.array[i]] },
                { sign: b.sign, array: Array.isArray(b.array[i]) ? [...b.array[i]] : [b.array[i]] }
            )
            if (cmpVal !== 0) return cmpVal
        }
    }
    // If no differences found, return equal
    return 0
}

/*
Dependencies : cmp
Accepts all valid gnum structures.
Returns a normalized form of the gnum, resolving irregularities
while maintaining perfect accuracy whenever possible.
*/
function normalizeInput(gnum) {
...
function compareGnums() {
    ...
    try {
        // Get sign values from radio buttons
        const sign1 = parseInt(document.querySelector('input[name="sign1"]:checked').value);
        const sign2 = parseInt(document.querySelector('input[name="sign2"]:checked').value);

        // Get array values from textareas
        const array1Input = document.getElementById('array1').value.trim();
        const array2Input = document.getElementById('array2').value.trim();

        if (!array1Input || !array2Input) {
            resultElement.textContent = "Please enter array values for both gnums";
        ...
        try {
            ...
            resultElement.textContent = "Error: Invalid array format for Gnum 1. Use JSON format like [10] or [10, 2]";
            resultElement.className = "error";
            return;
        }
        ...
        try {
            ...
            resultElement.textContent = "Error: Invalid array format for Gnum 2. Use JSON format like [10] or [10, 2]";
            resultElement.className = "error";
            return;
        }
        ...
        if (!Array.isArray(array1)) {
            resultElement.textContent = "Error: Gnum 1 array must be a valid array";
            resultElement.className = "error";
            return;
        }

        if (!Array.isArray(array2)) {
            resultElement.textContent = "Error: Gnum 2 array must be a valid array";
            resultElement.className = "error";
            return;
        }

        // Create gnum objects
        const gnum1 = { sign: sign1, array: array1 };
        const gnum2 = { sign: sign2, array: array2 };

        // Compare the gnums
        const result = cmp(gnum1, gnum2);
        const endTime = performance.now();
        const runtime = endTime - startTime;

        if (result === undefined) {
            resultElement.textContent = "Error: Unable to compare these values";
            resultElement.className = "error";
        } else if (result > 0) {
            resultElement.textContent = "Gnum 1 is greater than Gnum 2";
            resultElement.className = "success";
        } else if (result < 0) {
            resultElement.textContent = "Gnum 1 is less than Gnum 2";
            resultElement.className = "success";
        } else {
            resultElement.textContent = "Both gnums are equal";
            resultElement.className = "success";
        }

        // Display runtime
        runtimeElement.textContent = `Comparison completed in ${runtime.toFixed(2)} milliseconds`;
        ...
        resultElement.className = "error";
        runtimeElement.textContent = `Error occurred after ${runtime.toFixed(2)} milliseconds`;
    }
}

// Add event listeners for real-time updates
document.addEventListener('DOMContentLoaded', function() {
    const sign1Radios = document.querySelectorAll('input[name="sign1"]');
    const sign2Radios = document.querySelectorAll('input[name="sign2"]');
    const array1Input = document.getElementById('array1');
    const array2Input = document.getElementById('array2');

    // Handle zero sign selection - automatically set array to [0]
    function handleZeroSign(signRadios, arrayInput) {
        signRadios.forEach(radio => {
            radio.addEventListener('change', function() {
                if (this.value === '0' && this.checked) {
                    arrayInput.value = '[0]';
                    if (array1Input.value.trim() && array2Input.value.trim()) {
                        compareGnums();
                    }
                }
            });
    ...
    // Handle array editing when zero is selected - switch to positive
    function handleArrayEdit(arrayInput, signRadios) {
        arrayInput.addEventListener('input', function() {
            const zeroRadio = Array.from(signRadios).find(r => r.value === '0');
            const positiveRadio = Array.from(signRadios).find(r => r.value === '1');

            if (zeroRadio && zeroRadio.checked && this.value.trim() !== '[0]') {
    ...
    // Add event listeners to all inputs for automatic comparison
    function addAutoCompare(element) {
    ...
    // Set up zero sign handling for both gnums
    handleZeroSign(sign1Radios, array1Input);
    handleZeroSign(sign2Radios, array2Input);

    // Set up array editing handling for both gnums
    handleArrayEdit(array1Input, sign1Radios);
    handleArrayEdit(array2Input, sign2Radios);

    sign1Radios.forEach(addAutoCompare);
    sign2Radios.forEach(addAutoCompare);

    // Initial comparison if both arrays have values
    if (array1Input.value.trim() && array2Input.value.trim()) {
        compareGnums();
    }
});
